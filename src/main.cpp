...
#if defined(NDEBUG)
#error "Divc cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in udivc) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 350000 * COIN; // Premine
    }
    else if (nHeight <= 10000000 && nHeight >= 111111) {
      nSubsidy = 5 * COIN;
    }
    else {
        nSubsidy = 17.5 * COIN;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // DIVC: We use certain sporks during IBD, so check to see if they are
